...
class DataStoreSpider(scrapy.Spider):
    name = "localstore-spider"
    homeURL = "https://www.sn3studio.com"
    subcat_item = SubcategoryItem()

        

    def start_requests(self):
        start_urls = ['https://www.sn3studio.com/collections/all']
        for url in start_urls:
            item = StoreItem()
            yield scrapy.Request(url = "https://www.sn3studio.com/collections/all", callback=self.parse , meta={'item': item})
    ...
    # 
        ...
        mainclass = "#ajaxSection > x-cell"
        for xcell in response.css(mainclass):
            itemurl=xcell.css("div.card-image > a.no-style::attr(href)").get()
